...
void Controller::_hopForward()
{
    const HopParams hop = {
        ...
        .hopBackTime = 0.09f,.hopTime = 0.45f,.hopStopTime = 0.05f,.retriveTime = 1.2f,.restTime = 0.9f,
        .start_x = 9.41,.start_y = -5,.start_z = -10,//预备点
        .back_y = -5,.back_z = -10,
        .hop_y = -35,.hop_z = -31,//蹬腿点
        .end_y = 25,.end_z = -20,//结束点
        .bz_y1 = -25,.bz_z1 = -27,
        .bz_y2 = -30,.bz_z2 = 1
    };
...
void Controller::_hopForward_on_stair()
{
	...
	while(timer < hoponstair.restTime)
	{
		...
		for(int i = 0;i<4;++i)
		{
			if(i == 0 || i == 1)
			{
				params[i].feetPosY = hoponstair.end_fy*(1.0f-t)*(1.0f-t)+t*t*pos[i].y;
				params[i].feetPosZ = hoponstair.end_fz*(1.0f-t)*(1.0f-t)+t*t*pos[i].z;
			}
			else
			{
...
void Controller::_lerpRestore()
{
    ...
    m_planner.Update(0,pos_new,preserve,false,0,0);
