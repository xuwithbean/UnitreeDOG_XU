        ...
        }else if(m_status.auto_)
        {
            ...
            switch(cmd)
            {
                ...
                case 'R':
                {
                    ...
                    m_Lazer->uplazerfirst();
                    for(int i;i<=10000;i++)
                    {m_Lazer->getlazerparaminitfirst();}
                    for(int i;i<=10000;i++)
                    {m_Lazer->transferinit(m_Lazer->lazer_param.yaw);}
                    Record(m_pCtrl,m_Lazer);
...
Console::Console(const char* strKeyEvent,AutoCtrl* pCtrl,Controller* pCtrller,Lazer* pLazer)
{
