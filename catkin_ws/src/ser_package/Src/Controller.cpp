...
void Controller::Start(float startUpTime)
{
	...
	m_planner.Update(0, m_pos, m_touchStatus,false,0,0);
    m_planner.DebugShow();
    for (int i = 0; i < 4; ++i)
    {
...
bool Controller::Update(float velX, float velY, float velYaw,bool Hop,HopType type,bool restrictHop,bool climb,float climbangle,float yawangle)
{
	if(climb == false)
	{
		climbangle = 0;
	...
	LegController::CtrlParam param;
	bool status = m_planner.Update(dt, m_pos, m_touchStatus,climb,climbangle,yawangle);
	if(status)
	{
		if(m_needStop)
		{
			if(!m_moving)
			{
				...
				m_needStop = false;
				m_planner.Reset();
				m_planner.Update(0,m_pos, m_touchStatus,climb,climbangle,yawangle);
			}
		}
	}
	if (m_bVMCCtrl)
