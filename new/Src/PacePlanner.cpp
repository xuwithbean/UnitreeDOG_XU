...
void PacePlanner::SetVelocity(float forward,float vertical,float rotation)
{
    ...
    if(a == 1)
    {
    stepRot = rotation  * 1.5 * m_maxRotation * m_dogRadius;
    }
    else
    {
...
bool PacePlanner::Update(float deltaTime,std::vector<FeetMovement>& move,std::vector<bool>& touch,bool climb,float climbangle)
{
    float centerY = -5;
    bool loopOver = false;
